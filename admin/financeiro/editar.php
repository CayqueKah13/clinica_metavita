  ...
  <div class="col-12 mb-4">
    <div class="card card-cascade narrower">
      <div class="card-body card-body-cascade">
        ...
        <?php if ($item['installment'] > 0) { ?>
        <p>
          Parcela <?= $item['installment'] ?>/<?= $item['total_installments'] ?>
          <br>
          <?= Helper::decimalToBrlMoney($item['value']*$item['total_installments']) ?> (<?= $item['total_installments'] ?>x de <?= Helper::decimalToBrlMoney($item['value']) ?>)
        </p>
        <?php } ?>
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            <form class="text-center" method="post" action="_edit">
              ...
              <?php if ($item['id_group'] == 1) { ?>
                <select class="mdb-select md-form" name="supplier">
                  <option disabled selected>Fornecedor</option>
                  <?php foreach ($suppliersList as $key => $value) { ?>
                    <option value="<?= $value['id'] ?>" <?php if ($value['id'] == $item['id_supplier']) { echo "selected"; } ?>><?= $value['title'] ?></option>
                  <?php } ?>
                </select>
              <?php }?>
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="md-form">
                    <input type="text" id="input-price" class="form-control money" name="value" value="<?= $item['value'] ?>" required <?php if ($item['id_payment_method'] == 7) { echo "disabled"; } ?>>
              ...
              <select class="mdb-select md-form" name="confirmed">
                <option disabled selected>Status</option>
                <option value="0" <?php if ($item['is_confirmed'] == 0) { echo "selected"; } ?>>Pendente</option>
                <option value="1" <?php if ($item['is_confirmed'] == 1) { echo "selected"; } ?>>Confirmado</option>
              </select>
...
<?php if ($item['installment'] > 0) { ?>
<p class="text-center">
  ATENÇÃO! Este é um registro parcelado. ao excluir este registro o sistema excluirá todas as parcelas deste pagamento.
</p>
<?php } ?>
