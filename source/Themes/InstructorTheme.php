...
class InstructorTheme {

  // Admin Theme
  const TITLE = "MetaVita - Área do Profissional";
  const BASE_COLOR = "#064288";
  const FAV_ICON = Config::BASE_URL_INSTRUCTOR . "/_theme/img/fav_icon.png";
  const LOGIN_BG_IMAGE = Config::BASE_URL_INSTRUCTOR . "/_theme/img/admin-bg-2.jpg";
  const LOGIN_BG_COLOR = "#000000";
  const LOGIN_CARD_BG_COLOR = "rgba(40,40,40,0.5)";
  const LOGIN_CARD_COLOR = "#ffffff";
  const MENU_LOGO = "";
  // const MENU_BG = Config::BASE_URL_INSTRUCTOR . "/_theme/img/menu-bg.jpg";
  const MENU_BG = "";
  ...
  static function sideMenuItem($sufix, $tab, $currentTab, $iconName, $title) {
    $item = '<li><a href="'.Config::BASE_URL_INSTRUCTOR.'/'.$sufix.'" class="collapsible-header waves-effect ';
    if ($tab == $currentTab) {
      $item .= 'active';
    }
  ...
  static function avatarThumb($img, $title) {
    ...
    if ($i == "") {
      ...
      $color = InstructorTheme::colorForLetterThumb($letter);
